...
+ `item_id` 是后端提供的稳定学习对象 ID。前端可继续现有 lemma 方案；在生成 reading log 时，把对应 mark 的 `item_id` 一并回传即可。
...
+ `item_id`** 字段**：记录该表层形式对应的后端学习对象。前端不需要改变原有 lemma 逻辑，只需在 reading log 中带回该字段，供后端稳定更新 FSRS。
