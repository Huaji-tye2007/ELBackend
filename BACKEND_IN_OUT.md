...
LLM 会自然地写出屈折形式（consuming、consumed）。你在 prompt 里要求 LLM 在生成完文本后，附带输出一个词汇对照表：

或者更简单，让 LLM 直接在生成 messages 的同时输出 marks 草稿，标注表层形式和对应的 lemma。

第二步：Vocabulary Annotator 拿到草稿后

```python
surface_form = "consumed"      # LLM 给的，填入 marks.word
lemma = "consume"              # LLM 给的，或用 ecdict 反查
...
关键点：UserVocabulary 里的 VocabularyItem 存的 word 字段是 lemma（consume），不是屈折形式。Lemma 到 item_id 的映射在加载时顺便建好：

```python
lemma_index = {item["word"]: item["id"] for item in user_vocab["vocabulary"]}
# {"consume": "consume_1", "bank": "issue_problem", ...}
```
...
lemma_index   # lemma   → item_id，给 Vocabulary Annotator 用
```
